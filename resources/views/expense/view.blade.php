        ...
        <div class="col-xl-12 col-lg-12 col-sm-12">
            <div class="card shadow mb-4">
                ...
                <div
                    ...
                    <h6 class="m-0 font-weight-bold text-primary">Expense Recording</h6>
...
<div class="modal fade" id="ubahModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="labelmodalubah">Ubah Data Expense</h5>
            <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
            </button>
        </div>
        ...
        <div class="modal-body">
            <!-- Form untuk input -->
            <form action="#" class="formubahexpense" method="post">
            @csrf
            <input type="hidden" id="idexpensehidden" name="idexpensehidden" value="">
            <input type="hidden" id="tipeproses" name="tipeproses" value="">
                ...
                <div class="mb-6 row">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" id="description" name="description" placeholder="Masukkan Deskripsi, cth: Bayar Listrik">
...
<script>
      $(function(){
            $('.tampilmodaltambah').on('click', function(){
              // merubah label menjadi Tambah Data expense
              $('#labelmodalubah').html('Tambah Data Expense');
            //   url = "{{url('expense')}}";
			  url  = "{{ route('expense.store') }}";
              $('.formubahexpense').attr('action',url);
              ...
              $.ajax(
                {
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 400) {
                            ...
                            dataexpense(); //refresh data Pengeluaran
                        }
                    }
...
<script>
      function updateConfirm(e){
        ...
        $('#labelmodalubah').html('Ubah Data Expense');
        url = "{{ route('expense.store') }}";
        $('.formubahexpense').attr('action',url);
        $('#idexpensehidden').val(id);
        $('#tipeproses').val('ubah');
        $('#ubahModal').modal('show');

        var url3 = "{{url('expense/edit/')}}";
        var url4 = url3.concat("/",id);
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 404) {
                    ...
                    // console.log(response.expense.tanggal);
                    $('#date').val(response.expense.date);
                    $('#amount').val(response.expense.amount);
                    $('#description').val(response.expense.description);
                    $('#category').val(response.expense.category);
                    $('#wallet').val(response.expense.wallet);
                    $('#idexpensehidden').val(id)
...
<script>
        ...
        $(document).ready(function()
            {
                $('.formubahexpense').submit(function(e)
                    {
                        e.preventDefault();
                            $.ajax(
                                {
                                    ...
                                    success: function (response){
                                        ...
                                        if (response.status == 400) {
                                            ...
                                            // kosongkan form
                                            $('#ubahModal').modal('hide');
                                            dataexpense(); //refresh data expense
